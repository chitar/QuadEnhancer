...
class SAWLayer(nn.Module):
    ...
    def forward(self, result: Tensor) -> Tensor:
        ...
        if self.saw_method=='roll' and self.saw_k>0:
            ...
            # result = checkpoint(roll_opt,result, self.saw_theta_0)
            #
            ...
            shapes = result.shape
            result = result.permute(0,2,3,1).reshape(-1,self.out_features)
            x = self.saw_Q(self.saw_P(result))
            x = x / (1 + x.norm(dim=-1, keepdim=True))
            result = (x + 1) * result
            result = result.reshape(shapes[0],shapes[2],shapes[3],shapes[1]).permute(0,3,1,2)
        return result
...
class _BaseConvNormActLayer(BaseLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if saw_method != 'linear' and saw_k > 0:
            saw_layer = SAWLayer(out_channels)
            self.saw_method = saw_method
            self.saw_k = saw_k
            block.add_module(name='saw_layer',module=saw_layer)
    ...
    def __repr__(self):
        ...
        if saw_method is not None:
            repr_str += ", saw_method={}, saw_k={}".format(saw_method, saw_k)
